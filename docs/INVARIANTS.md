...
The rating prompt runs within a successful picker/settings UI action, never on the command worker after the UI dispatcher returns. SettingsStore marks it asked in one mutation, preserving concurrent usage updates; subsequent connections stop recording rating-prompt usage.
