...
AdaptiveResourceController owns pressure observation, policy, retry/schedule and the published ResourceStatusSnapshot. Its snapshot mutex protects only pressure authorization sequence and snapshot copies. The host retains the owner from construction through final controller reads; Start/Stop and window messages run on UI. Stop closes admission, drains the monitor, invalidates timers and releases tray/dispatcher references. Weak callbacks cannot retain the host or form a tray cycle. Evaluation retains the tray across native picker calls, which can reenter Stop; it checks admission again before publishing or scheduling. Convert policy/pressure once on publication; readers copy the same value. Pure policy has no host retry/scheduling dependencies.
