...
| 6 Tray | Build snapshot-driven `TrayUi`/rows and one flyout lifecycle state machine; preserve theme/DPI/clicks and resource facts; implement P03; remove direct settings/device access. | No animation timer/frame; four static states pass themes/DPI; picker and P02 UI scenarios pass. |
...
| 11 Cleanup | Remove adapters/dead files; colocate source; update projects/docs; add dependency checks; record final size/performance. | No unowned bridge or duplicate owner; full [verification](verification.md) passes; source map matches [architecture](architecture.md). |
