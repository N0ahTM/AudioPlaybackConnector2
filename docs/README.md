# Documentation

Read only the page/section relevant to the task.

| Task | Reference |
| --- | --- |
| Build, test, dependencies, contribution | [CONTRIBUTING](../CONTRIBUTING.md) |
| Find components and ownership | [Architecture](ARCHITECTURE.md) |
| Change async, locks, callbacks or shutdown | [Runtime invariants](INVARIANTS.md) — affected owner only |
| Review a reported defect | [Bugfix tracking](BUGFIXES.md) — affected IDs; German |
| Package, verify or release | [Releasing](RELEASING.md) |
| Use the app / automate it | [Usage](USAGE.md) / [CLI](CLI.md) |
| Install / diagnose | [Installation](INSTALLATION.md) / [Troubleshooting](TROUBLESHOOTING.md) |
| Translate | [Translating](TRANSLATING.md) |
| Historical behavior / vulnerability report | [Changelog](../CHANGELOG.md) / [Security](../SECURITY.md) |

[Discussions](https://github.com/N0ahTM/AudioPlaybackConnector2/discussions): questions and recipes. [Issues](https://github.com/N0ahTM/AudioPlaybackConnector2/issues): reproducible defects.
