...
Only schemaVersion 2 is accepted and emitted. Validate the whole input, including unknown/duplicate keys or identities, types and limits; optional omissions use defaults. JSON DOM remains private. Only file/path-not-found means absent. Empty or unreadable files fail preservation. A failed preservation closes mutation admission before load releases it; mutations reject and flush fails for that instance. Mutations wait for load before deciding. Successful preservation permits defaults.
