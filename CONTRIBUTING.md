...
For an indirect lock-call investigation, run `python -B scripts/test-audit-owner-locks.py`, then `python -B scripts/audit-owner-locks.py <CoreRuntime compile_commands.json> <App compile_commands.json> --output <report.json>` using the databases generated by the ClangTidy targets. The report is a candidate list for owner-by-owner review, not a passing gate: it does not model every path, implicit destructor, header-only body or native SRW lock.
